...
``GET /api/v3/worker/{id}/routes``, ``POST /api/v3/worker/{id}/refresh-token``,
``GET /api/v3/worker/{id}/secrets/{ref}``) spoken by the Continuum Router
binary.
...
from .secrets import SecretFetchResponse
