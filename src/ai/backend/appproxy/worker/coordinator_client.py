...
def _bearer_headers(root_ctx: RootContext, request_id: str) -> dict[str, str]:
    """Headers for v3 calls that follow registration.

    The coordinator's ``worker_v3_auth_required`` decorator expects an
    ``Authorization: Bearer <poll_token>`` header — the token is the one
    issued in the register response and stored on ``root_ctx``. We add the
    request-id header so coordinator access logs can be correlated, but
    do NOT echo ``api_secret`` here: v3 endpoints reject it outside the
    bootstrap path.
    """
    if root_ctx.poll_token is None:
        raise WorkerRegistrationError(
            extra_data={"reason": "v3 call attempted before register_worker stored poll_token"}
        )
    return {
        "X-BackendAI-RequestID": request_id,
        "Authorization": f"Bearer {root_ctx.poll_token}",
    }
...
async def register_worker(root_ctx: RootContext, request_id: str) -> None:
    ...
    body. Bootstrap auth is the long-lived ``X-BackendAI-Token:
    <api_secret>`` header; the 200 response carries a freshly-issued
    ``poll_token`` (HS256 JWT) which the worker stashes on ``root_ctx``
    and reuses for heartbeat / deregister. Re-registration overwrites the
    stored token transparently.
    """
    local_config = root_ctx.local_config
    sess = _get_coordinator_session(root_ctx)
    headers = _get_request_headers(root_ctx, request_id)
    ...
    try:
        async with sess.post("/api/v3/worker", json=body, headers=headers) as resp:
            ...
            root_ctx.poll_token = response.poll_token
            log.debug(
                "Joined to coordinator {} (worker_id={}, token_revision={})",
                root_ctx.local_config.proxy_worker.coordinator_endpoint,
                root_ctx.worker_id,
                response.token_revision,
            )
    except aiohttp.ClientResponseError as e:
        log.exception("")
        if e.status == 400:
...
async def deregister_worker(root_ctx: RootContext, request_id: str) -> None:
    ...
    sess = _get_coordinator_session(root_ctx)
    headers = _bearer_headers(root_ctx, request_id)
...
async def ping_worker(root_ctx: RootContext, request_id: str) -> None:
    ...
    try:
        ...
        if e.status == 401:
            # The stored poll_token has expired or been revoked. Drop it
            # so the caller (server.py heartbeat loop) re-registers via
            # the bootstrap ``api_secret`` path on the next iteration.
            root_ctx.poll_token = None
            raise CoordinatorConnectionError(
                extra_data={
                    "authority": worker_config.authority,
                    "coordinator": str(worker_config.coordinator_endpoint),
                    "reason": "poll_token expired or revoked; re-register required",
                }
            ) from e
        raise
    except ClientConnectorError as e:
